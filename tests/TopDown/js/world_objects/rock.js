export class Rock {
  constructor(x, y) {
    this.x = x;
    this.y = y;
    this.width = 45;
    this.height = 45;
    this.progress = 0;
    this.interactable = true;
    this.miningPhase = 0;
    this.type = 'stone';
    this.health = 2;
    this.tooltipContent = {
      title: 'Stone Rock',
      type: 'Stone',
      description: 'A solid mineral deposit\nCan be mined for stone resources'
    };
    
    // Generate main rock shape points
    this.points = [
      { x: -20, y: -5 },
      { x: -15, y: -15 },
      { x: -5, y: -20 },
      { x: 5, y: -18 },
      { x: 15, y: -10 },
      { x: 18, y: 0 },
      { x: 15, y: 10 },
      { x: 5, y: 15 },
      { x: -5, y: 12 },
      { x: -15, y: 5 }
    ];
    
    // Generate random crack points
    this.cracks = [
      { start: { x: -10, y: -5 }, cp: { x: -5, y: 0 }, end: { x: 5, y: 5 } },
      { start: { x: 5, y: -10 }, cp: { x: 0, y: -5 }, end: { x: -5, y: 0 } },
      { start: { x: -8, y: 8 }, cp: { x: 0, y: 5 }, end: { x: 8, y: 2 } }
    ];
  }

  checkClick(worldX, worldY) {
    return worldX > this.x - this.width/2 && 
           worldX < this.x + this.width/2 &&
           worldY > this.y - this.height/2 && 
           worldY < this.y + this.height/2;
  }

  startMining() {
    if (this.progress < 100) {
      this.progress += 25;
      this.miningPhase = performance.now();
    }
  }

  render(ctx) {
    ctx.save();
    ctx.translate(this.x, this.y);

    // Draw shadow
    ctx.fillStyle = 'rgba(0,0,0,0.3)';
    ctx.beginPath();
    ctx.ellipse(3, 3, 20, 12, 0, 0, Math.PI * 2);
    ctx.fill();

    // Draw main rock shape
    ctx.beginPath();
    ctx.moveTo(this.points[0].x, this.points[0].y);
    for (let i = 1; i < this.points.length; i++) {
      ctx.lineTo(this.points[i].x, this.points[i].y);
    }
    ctx.closePath();

    // Create base gradient
    const baseGradient = ctx.createRadialGradient(-5, -5, 0, 0, 0, 25);
    baseGradient.addColorStop(0, '#a0a0a0');  // Light gray
    baseGradient.addColorStop(0.4, '#808080'); // Medium gray
    baseGradient.addColorStop(1, '#606060');   // Dark gray
    
    ctx.fillStyle = baseGradient;
    ctx.fill();
    
    // Add stone texture
    ctx.strokeStyle = '#505050';
    ctx.lineWidth = 0.5;
    
    // Draw detailed cracks
    this.cracks.forEach(crack => {
      // Main crack
      ctx.beginPath();
      ctx.moveTo(crack.start.x, crack.start.y);
      ctx.quadraticCurveTo(crack.cp.x, crack.cp.y, crack.end.x, crack.end.y);
      ctx.strokeStyle = '#505050';
      ctx.lineWidth = 1.5;
      ctx.stroke();
      
      // Smaller branch cracks
      ctx.lineWidth = 0.5;
      for (let i = 0; i < 2; i++) {
        const t = 0.3 + i * 0.4;
        const px = this.quadraticPoint(crack.start.x, crack.cp.x, crack.end.x, t);
        const py = this.quadraticPoint(crack.start.y, crack.cp.y, crack.end.y, t);
        
        ctx.beginPath();
        ctx.moveTo(px, py);
        ctx.lineTo(
          px + (Math.random() - 0.5) * 8,
          py + (Math.random() - 0.5) * 8
        );
        ctx.stroke();
      }
    });

    // Add surface details
    for (let i = 0; i < 15; i++) {
      ctx.beginPath();
      const angle = Math.random() * Math.PI * 2;
      const radius = Math.random() * 15;
      const x = Math.cos(angle) * radius;
      const y = Math.sin(angle) * radius;
      ctx.arc(x, y, 1, 0, Math.PI * 2);
      ctx.fillStyle = Math.random() > 0.5 ? '#757575' : '#656565';
      ctx.fill();
    }

    // Add highlights
    ctx.fillStyle = 'rgba(255,255,255,0.15)';
    ctx.beginPath();
    ctx.ellipse(-8, -8, 6, 4, Math.PI/4, 0, Math.PI * 2);
    ctx.fill();

    // Add outline
    ctx.strokeStyle = '#404040';
    ctx.lineWidth = 1.5;
    ctx.stroke();

    // Mining progress bar
    if (this.progress > 0) {
      const barWidth = 40;
      const barHeight = 8;
      
      // Progress bar background
      ctx.fillStyle = 'rgba(0,0,0,0.8)';
      ctx.fillRect(-barWidth/2, -35, barWidth, barHeight);
      
      // Progress bar border
      ctx.strokeStyle = '#ffffff';
      ctx.lineWidth = 1;
      ctx.strokeRect(-barWidth/2, -35, barWidth, barHeight);
      
      // Progress fill
      const progressGradient = ctx.createLinearGradient(-barWidth/2, 0, barWidth/2, 0);
      progressGradient.addColorStop(0, '#8B4513');
      progressGradient.addColorStop(1, '#A0522D');
      ctx.fillStyle = progressGradient;
      ctx.fillRect(-barWidth/2, -35, (barWidth * this.progress)/100, barHeight);
      
      // Mining sparkle effect
      if (this.progress >= 75) {
        const sparkleOpacity = 0.5 + Math.sin(performance.now() / 200) * 0.5;
        const sparkleSize = 2 + Math.sin(performance.now() / 150);
        
        ctx.fillStyle = `hsla(50, 100%, 70%, ${sparkleOpacity})`;
        ctx.beginPath();
        ctx.arc(
          -barWidth/2 + (barWidth * this.progress/100) - 4,
          -35 + barHeight/2,
          sparkleSize,
          0, Math.PI*2
        );
        ctx.fill();
      }
    }

    ctx.restore();
  }

  getTooltipContent() {
    return this.tooltipContent;
  }

  // Helper function for quadratic curve point calculation
  quadraticPoint(p0, p1, p2, t) {
    return (1 - t) * (1 - t) * p0 + 2 * (1 - t) * t * p1 + t * t * p2;
  }
}